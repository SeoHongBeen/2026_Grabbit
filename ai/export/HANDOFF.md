...
2. **알림 시점의 5초 버퍼 중 가장 시끄러운 구간**을 골라 씁니다. 연속 조건
   때문에 알림은 소리가 난 뒤 1~2창 지나서 울려서, 마지막 청크만 보면
   정작 그 소리가 없을 수 있습니다.
...
| 클래스 | 임계값 | 연속 | 요구 지속시간 (hop 1초) |
|---|---|---|---|
| `glass_breaking` | 0.30 | 2회 | 2.0초 |
| `siren` | 0.30 | 2회 | 2.0초 |
| `door_wood_knock` | 0.55 | 1회 | 1.0초 |
| `doorbell` | 0.30 | 2회 | 2.0초 |

유리·노크·초인종은 1~2초짜리라 조건을 높이면 아예 안 울립니다.
이 값들은 실환경 녹음 2시간을 분석해 정한 것입니다.

**체감 지연 = 요구 지속시간 + 처리 시간.** 노크 약 1.5초,
...
다시 잡겠습니다 — `--hop` 만 만지면 알림이 조용히 늦어지거나 밀립니다.
